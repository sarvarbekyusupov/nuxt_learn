...
import { createForgotPasswordSchema } from '@/utils/auth/validation'
...
const { handleSubmit, isSubmitting } = useForm({ validationSchema: createForgotPasswordSchema(t) })

const onSubmit = handleSubmit(async (_values) => {
  try {
    await new Promise(resolve => setTimeout(resolve, AUTH_API_CONFIG.MOCK_DELAY_MS))
    message.success(t('auth.messages.resetLinkSent'))
  } catch {
  ...
  <div class="page">
    <div class="card">
      ...
      <p class="footer">
        ...
        <router-link :to="localePath(ROUTES.LOGIN)">{{ t('auth.signInLinkForgot') }}</router-link>
      </p>
    </div>
  </div>
</template>
